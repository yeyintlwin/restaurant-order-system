...
| 2026-08-05 | Close-out audit: six documentation claims Task 17's greps could not reach — four in the slice spec carrying no deferral keyword (§6.2's *"the settled four"*, §9's opening, §9.1 B3, §9.3's line budget), two outside its `--include=*.md --include=*.js` globs (`Dockerfile`, and the two `withServer` comments). All rewritten as records rather than deleted. **Task 16 Step 4's manual end-to-end check stays UNTICKED and parent §12's two boxes stay unticked** — the check needs a human at a terminal and no agent can stand in for one. What the suite proves without one, named so nobody re-derives it: the CLI **refuses a pipe** and exits non-zero naming the fix (`scripts.test.js`, *"refuses a piped password"* — `spawnSync` gives the child a pipe and never a TTY, which is the `docker compose exec -T` case exactly); the bootstrap guard is **monotonic across a delete** (`auth-users.test.js`, *"the guard is an audit row, not the current state"* — DELETE the `platform_admin` row, re-run, still refused, which is §12's own wording); the **boot warning still listens** on an empty `platform_admin` set and on a failed count (`server-bootstrap.test.js`, two tests — the deliberate exception to refuse-to-start, without which the container the operator must `exec` into would not be running); and a **real login returns 200 with the `__Host-` cookie**, full attribute string asserted, on a server actually listening (`auth-routes.test.js`). What it does **not** prove is **echo-off itself**: a pipe has no echo to switch off, so a `spawnSync` test can see the refusal and can never see the prompt. That needs a pty, and a pty is the one thing the harness has no way to allocate — which is why the box is manual rather than missing. | **17/17** | (this commit) | Phase 3 |
