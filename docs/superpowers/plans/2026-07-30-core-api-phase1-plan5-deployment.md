...
- **Task 10** — install and prove the nginx proxy. **DNS and the CERTIFICATE are both DONE as of
  2026-08-03**; what remains needs the two conf files to be on the box, which is Task 18's scp
  and Task 20's install, so Task 10 now genuinely runs at cutover rather than being blocked.
  Certificate: `CN=api.yeyintlwin.com`, issuer Let's Encrypt `YE1`, valid to **2026-11-01**, at
  `/etc/letsencrypt/live/api.yeyintlwin.com/`, renewal timer installed by certbot. Issued with
  `certbot certonly --nginx --non-interactive`, after a `--dry-run` against staging that passed;
  `nginx -t` was valid before and after both runs, so none of the thirteen `listen … 443` lines
  moved. **Task 10's item 1 ("the certificate exists before anything else") is satisfied.**
  Historical detail, kept because it was the blocking condition — DNS was verified
  ...
  **not** write it into any vhost. The `--nginx` authenticator edits the configuration only for
  ...
  of the blast radius.
- **Task 15 — DONE 2026-08-03. The drill has been run on the box and its box is ticked.** Four
  of the six pre-cutover boxes in `infra/README.md` are now ticked. The two still open are
  **Scenario A rehearsed against `core_scenario_a`** (a separate rehearsal, nobody's task in
  this plan) and **`crontab -l | grep -q backup-core-db.sh`**, which is Task 24's to install.
...
**Do not hand-increment the count in that column.** It was wrong by one from the Tasks 11–12 row
onward (Tasks 1–9 plus 11 and 12 is **11**, recorded as 12) and every later row inherited the
error, so the header claimed 17 while sixteen tasks were done. Derive it instead — a task is
finished when its `### Task N` section contains at least one `- [x] **Step` and no `- [ ] **Step`:

```bash
node -e 'const l=require("fs").readFileSync("docs/superpowers/plans/2026-07-30-core-api-phase1-plan5-deployment.md","utf8").split("\n");const h=[];l.forEach((s,i)=>{const m=s.match(/^### Task (\d+):/);if(m)h.push({n:+m[1],at:i})});const done=h.filter((x,k)=>{const e=k+1<h.length?h[k+1].at:l.length;let d=0,t=0;for(let i=x.at;i<e;i++){if(/^- \[x\] \*\*Step/.test(l[i]))d++;else if(/^- \[ \] \*\*Step/.test(l[i]))t++}return t===0&&d>0});console.log(done.length+"/"+h.length+" -> "+done.map(x=>x.n).join(","))'
```
...
| 2026-07-31 | **Tasks 5-9 — Part 2 code complete.** `infra/nginx/core-api-proxy.conf`, `api.conf` (zones, upstream, port 80, TLS block), the two structural XFF guards (both mutation-proven), and the nginx runbook. Confirmed review must-fix 9 and 10 against real shipped files: the original real_ip grep returns 1 by matching api.conf's own warning comment, and the original XFF grep returns 0 against the column-aligned snippet. | **9/30** | `e43ce04`, `8510d9f`, `8e56bb6`, `758dff9`, `b2a1dd7` | Task 10 |
...
> **DEFECT FOUND ON EXECUTION 2026-08-03, fixed in the text below.** Two Step-1 assertions were
> written with a literal space against prose that this file hard-wraps at ~95 columns, so the
> phrase straddles the wrap and the regex can never match a correct runbook:
> `/scrypt hashes/` versus `…and scrypt\nhashes**`, and
> `/only when it creates the data directory/i` versus `…the data\ndirectory**`. Both now use
> `\s+` at the wrap point. Mutation-tested: replacing either sentence turns exactly one test red.
>
> **This is must-fix 10's shape in a new file type** — that one was single spaces against a
> column-aligned nginx conf; this is single spaces against wrapped markdown. **Any prose
> assertion over a hard-wrapped document must treat inter-word space as `\s+`.** Rewrapping the
> prose instead would have been the fragile fix: the next reflow breaks it again for no
> semantic reason.
